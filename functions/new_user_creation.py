...
def set_pin():
    pin_set = False
    while not pin_set:
        set_pin = input("Please set a 4 digit numeric access pin: ")
...
def get_input():
    ...
    user = NewUser(id, data["First Name"], data["Last Name"], data["Date of Birth"], data["Occupation"],
                   data["Address"], data["Email"], pin)
